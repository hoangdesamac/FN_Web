...
// NEW: endpoint session-sync (thêm vào server-postgres.js sau các route auth)
app.get('/api/session-sync', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        // Lấy orders (chỉ meta/đếm hoặc list ngắn) — bạn có thể chỉnh theo nhu cầu
        const ordersRes = await pool.query(
            `SELECT id, order_code AS "orderCode", items, total, status, created_at AS "createdAt", unseen
             FROM orders
             WHERE user_id = $1
             ORDER BY created_at DESC
             LIMIT 50`,
        ...
        const addrRes = await pool.query(
            `SELECT id, recipient_name, recipient_phone, street_address, ward, city, is_default
        ...
        console.error('GET /api/session-sync error:', err);
        res.status(500).json({ success: false, error: 'Lỗi server khi đồng bộ phiên' });
    }
});
