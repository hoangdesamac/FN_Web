...
function buildCategoryList(){ const ul=document.getElementById('category-list'); ul.innerHTML=''; PART_CATEGORIES.forEach(c=>{ const li=document.createElement('li'); li.dataset.key=c.key; li.innerHTML=`<span>${c.label}</span><span class="count" id="count-${c.key}">0</span>`; li.addEventListener('click',()=> openPartModal(c.key)); ul.appendChild(li); }); }
...
function renderPartModalList(){
    ...
    if(q){
        list=list.filter(p=> (p.name&&p.name.toLowerCase().includes(q))||(p.socket&&p.socket.toLowerCase().includes(q))||(p.type&&p.type.toLowerCase().includes(q))||(p.chipset&&p.chipset.toLowerCase().includes(q)));
    }
...
function renderSelected(category){ const cell=document.getElementById('cell-'+category); const priceCell=document.getElementById('price-'+category); const actCell=document.getElementById('act-'+category); const countSpan=document.getElementById('count-'+category); const part=state.selected[category]; if(!part){ cell.innerHTML=`<button class="part-select-btn" data-key="${category}"><i class="fa fa-plus"></i> Chọn ${(PART_CATEGORIES.find(c=>c.key===category)||{}).label||category}</button>`; priceCell.textContent='-'; actCell.innerHTML=''; countSpan.textContent='0'; cell.querySelector('button').addEventListener('click',()=> openPartModal(category)); return; } countSpan.textContent='1'; cell.innerHTML=`<div class="selected-part"><div class="name">${part.name}</div><div class="meta">${buildMeta(part)}</div></div>`; priceCell.textContent=formatPrice(part.price); actCell.innerHTML=`<button class="remove-part-btn" onclick="removePart('${category}')">×</button>`; }
...
async function ensureCategory(cat){ if(loadedCats.has(cat)) return; const res=await fetchDataFile(cat + '.json'); if(!res){ loadedCats.add(cat); return; } try{ const arr=await res.json(); if(Array.isArray(arr)){ if(!PART_LIBRARY[cat]) PART_LIBRARY[cat]=[]; const existing=new Set(PART_LIBRARY[cat].map(p=>p.id)); let added=0; arr.forEach(it=>{ if(!it.id) it.id=cat+'_'+Math.random().toString(36).slice(2); if(existing.has(it.id)) return; PART_LIBRARY[cat].push(it); existing.add(it.id); added++; }); if(added) console.log('[PC BUILDER] Loaded', added, cat); } }catch(e){ console.warn('Processed parse fail', cat, e); } loadedCats.add(cat); }
...
async function initProcessed(){ buildCategoryList(); buildConfigRows(); preloadCore(); loadFromLocal(); Object.keys(state.selected).forEach(k=> renderSelected(k)); recalcTotals(); updateSummary(); const search=document.getElementById('part-search'); if(search) search.addEventListener('input', ()=> { currentModalLimit = MODAL_PAGE_SIZE; renderPartModalList(); }); document.getElementById('close-modal')?.addEventListener('click', closeModal); document.getElementById('part-modal')?.addEventListener('click', e=>{ if(e.target.id==='part-modal') closeModal(); }); document.getElementById('clear-config')?.addEventListener('click', clearConfig); document.getElementById('export-json')?.addEventListener('click', exportJSON); document.getElementById('save-config')?.addEventListener('click', saveToLocal); document.getElementById('load-config')?.addEventListener('click', ()=>{ loadFromLocal(); Object.keys(state.selected).forEach(k=> renderSelected(k)); recalcTotals(); updateSummary(); }); }
